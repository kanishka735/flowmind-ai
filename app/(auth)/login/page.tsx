...
import { Brain, Loader2, AlertCircle, User2 } from 'lucide-react'
...
function LoginContent() {
  ...
  useEffect(() => {
    if (guestParam === 'true' && !user && !loading) {
      handleGuestLogin()
    }
      ...
      >
        <div className="text-center mb-10">
          <motion.div
            ...
            className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-purple-500 to-teal-500 mb-4 shadow-glow-purple"
          >
          ...
          <div className="space-y-4">
            ...
            >
              {isGuestLoading ? (
                ...
                <User2 className="w-5 h-5 text-[#8B8BA7]" />
              )}
              {isGuestLoading ? 'Setting up...' : 'Continue as Guest'}
            </button>
          </div>

          <p className="text-center text-xs text-[#4A4A6A] mt-6 leading-relaxed">
            Guest mode stores data for this session only.
            <br />
        ...
        <p className="text-center text-xs text-[#4A4A6A] mt-6">
          Coding Ninjas × Google Vibe2Ship Hackathon 2026
        </p>
      </motion.div>
    </div>
  )
